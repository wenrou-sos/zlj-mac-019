...
| 排缸安排 | 待排缸列表 + 机台看板；**实时预检**：超容/机型不适配标红并说明原因（需人工确认方可排入）、同布种同色号给出拼缸建议、时段冲突提示下一可用空档；已排缸号改重量后自动重新校验并在看板标警；加急/特急缸号置顶标红并在排缸时弹优先提示 |
| 交期风险 | 订单列表/详情/排缸看板实时标出**落后计划的缸号**与**会拖期的订单**（已拖期/拖期风险），原因归因为卡在排缸、卡在工序或卡在异常返修；全部动态计算，随排缸调整、工序推进、异常处理自动刷新消失 |
